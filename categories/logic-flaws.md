...
---
### [wooyun-2015-0143700] 美的营销系统存xml注入（进一步可获取客户、员工、订单各种数据）
...
**厂商**: 华为技术有限公司 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
**POC**: 测试代码:测试手机: Huawei MT2ROM版本: MT2-L05V100R001C17B111Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE, null);startActivity(intent);使用以上代码将调用系统的自带相机功能，系统自带相机启动后大约在10-15秒(视环境温度而定)左右时间即会因为图像处理运算将手机温度逐渐提高至43℃，随后系统温度守护进程会因执行设计缺陷的mitigation功能导致系统相机崩溃。[备注:未测试其它华为手机，如果使用同样配置文件可能有相同的问题]
...
---
### [wooyun-2013-040837] 苏宁易购短信验证码短信轰炸
**厂商**: 江苏苏宁易购电子商务有限公司 | **年份**: 2013 | **类型**: 网络设计缺陷/逻辑错误
...
---
### [wooyun-2011-01188] 赶集网活动手机验证码发送缺陷和物理路径泄露
**厂商**: 赶集网 | **年份**: 2011 | **类型**: 设计缺陷/逻辑错误
...
**详情**: 疏忽1，360关闭主防的调用过于简单360关闭主动防御的方式是传参调用360tray，命令也比较简单，就是“360tray.exe /disablesp 2”如果用cmd调用该命令会被360拦截，但创建快捷方式（快捷方式的写法就是"路径\360tray.exe /disablesp 2"）点击调用则不会被拦，原因是360会验证调用的来源，来自explorer的调用的合法的，弹出的主防关闭询问对话框如下图所示：大家都知道，这样生成的快捷方式的图标是360的图标，鉴于快捷方式的图标是可以修改的，可以修改.lnk文件修改之。现在，我们得到了一个能打开360主防关闭询问对话框的快捷方式，记做launch.lnk。该快捷方式用两种方式皆可运行：一种是用程序打开所在文件夹后，模拟键盘按键调用。另一种是改成邪恶的名称比如“安装”，诱使用户点击。（360策略：调用explorer打开文件夹不会被拦截，调用
...
---
### [wooyun-2015-0144601] 同花顺爱基金客户端之账户密码找回功能的短信验证可被绕过
**厂商**: 同花顺 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 1.客户端基本信息此次测试采用的是Android客户端，下载地址：http://**.**.**.**/public/mobile/apk  : com.hexin.android.bank, 48, V4.30.03, 同花顺爱基金apkmd5: ea96923d1ab76255b1f4e6a076482d4a证书 :certmd5: 110582f43caed505854599c7ada28e77issuer: C=CN, ST=ZheJiang, L=Hangzhou, O=ShouJiKaiFaZu, OU=TongHuaShun, CN=hexinsubject: C=CN, ST=ZheJiang, L=Hangzhou, O=ShouJiKaiFaZu, OU=TongHuaShun, CN=hexin2.漏洞描述找回密码功能，服务端通过客户端输入动态短信验证码来鉴别身份，但
...
---
### [wooyun-2012-014706] 苏宁易购会员中心某验证功能权限设计错误
**厂商**: 江苏苏宁易购电子商务有限公司 | **年份**: 2012 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2012-011125] 联想开发者社区及论坛逻辑漏洞，可获得管理员权限
**厂商**: 联想 | **年份**: 2012 | **类型**: 设计缺陷/逻辑错误
...
**详情**: 在联想开发者社区注册新用户时，竟然可以间接注册admin帐号，而注册的帐号又可以在开发者论坛“共用”，结果导致修改了开发者论坛的admin帐号信息（密码等），然后.....
...
**POC**: 拷贝360主动防御主程序（或者360的其他程序）到某目录，在同目录放置WSOCK32.dll即可，WSOCK32.dll里的dllMail函数里可以执行恶意代码，整个行为将被360安全卫士放过，其他如NETAPI32.dll可能也同样有效
...
---
### [wooyun-2014-053181] 再次挖掘乐视商城支付逻辑漏洞
**厂商**: 乐视网 | **年份**: 2014 | **类型**: 设计缺陷/逻辑错误
...
**修复**: 应该好好修复一下了.乐视是个上市企业，应该注重一点了吧。修复什么的你们比我懂！
---
...
---
### [wooyun-2015-0107132] 苏宁易购Android客户端逻辑缺陷导致隐私泄漏
...
**详情**: 苏宁易购Android版对file域下符号链接限制不当，可导致cookie等用户隐私被窃取
...
**详情**: 进入中国银行手机客服端的主菜单，点击账单缴付-自助缴付，选择自己的网银账号后，只要输入手机号码的归属地，选择通讯费，选择号码的运营商（前提该号码运营商在该市跟中国银行有合作关系），输入手机号码后就会显示客户的姓名以及话费余额每个市可以验证的号码有不同，随州的移动和电信都可以，武汉市只有移动
...
**厂商**: 杭州昵图信息技术有限公司 | **年份**: 2014 | **类型**: 内容安全
...
---
### [wooyun-2013-037423] 米聊MiTalk存在DLL劫持漏洞
**厂商**: 小米科技 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 测试时米聊的安装路径为：C:\Program Files\Xiaomi\MiTalk\打开MiTalk.exe时会加载三个处不存在的DLL文件，分别是：C:\Program Files\Xiaomi\MiTalk\mitalkchs.dllC:\Program Files\Xiaomi\MiTalk\mitalkcht.dllC:\Program Files\Xiaomi\MiTalk\mitalkloc.dll以上任一DLL文件与MiTalk.exe放在对应的目录，这样就可以在运行MiTalk.exe的时候会同时加载对应的DLL，这个DLL可以为自制的任意内容的dll。此方法可用于劫持运行某些程序（木马病毒）。（我这里放入特定的dll后，运行MiTalk.exe会弹出一个特定的框显示"Hello from MyDLL!"，如下图）
...
---
### [wooyun-2012-04010] 360安全浏览器设计缺陷-误报弹窗广告
**厂商**: 奇虎360 | **年份**: 2012 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 如果用鼠标中键单击链接,将被360安全浏览器拦截为弹窗广告
...
---
### [wooyun-2015-0100887] 今日头条APK两处逻辑漏洞
**厂商**: 字节跳动 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2013-041617] 联想某积分商城支付漏洞再绕过
**厂商**: 联想 | **年份**: 2013 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0105688] 联想手机VIBE UI部分版本设计缺陷绕过锁屏读所有联系人
**厂商**: 联想 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
**修复**: 方法一：把逻辑设置为，自定义回复短信的时候需要输入解锁密码方法二：学习华为的最新系统，把在通话界面实现自定义回复短信的功能用另外的模块实现，在这个模块中只有短信内容框别的什么都没有。
---
...
---
### [wooyun-2012-014541] 苏宁易购手机版 易付宝支付逻辑缺陷
**厂商**: 江苏苏宁易购电子商务有限公司 | **年份**: 2012 | **类型**: 账户体系控制不严
...
---
### [wooyun-2015-0159025] 海尔集团某站逻辑漏洞泄漏用户敏感信息（姓名+身份证+邮箱+手机）
...
**详情**: 随手挑了一个杯具来团购，恩来到网银支付前的最后一个页面，然后开启firebug或者用chrome，把页面里的和价格有关的39统统改成了1块钱，然后提交跳转到招行页面之后，果然只要支付1块钱

**POC**: 跳转到招行页面之后，果然只要支付1块钱
...
**厂商**: 北京士惠农业发展有限公司 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2014-082114] 苏宁易购某网站全网号码存在任意登录、短信轰炸、绕过验证码的漏洞
...
**厂商**: 江苏苏宁易购电子商务有限公司 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
**厂商**: 苏州同程旅游网络科技有限公司 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2013-039011] 小米锁屏设计缺陷，可利用后绕过手机安全锁，任意操作用户手机
**厂商**: 小米科技 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**修复**: 修补：还是做一下权限的判定设计问题吧，将第三方软件的高权限请求拒之门外吧！最后再喊一次，有么有小米3手机？米3F码也可以啊！
---
...
**详情**: 中兴微品http://**.**.**.**随便选择一件商品 结算然后进入到提交 订单  直接提交在下一步
...
---
### [wooyun-2015-0138651] 苏宁易购某分站严重逻辑缺陷（让任意商品下架）
...
---
### [wooyun-2015-096239] 海尔短信平台万能密码进后台(各种敏感信息泄露可做短信轰炸机)
**厂商**: 海尔集团 | **年份**: 2015 | **类型**: SQL注射漏洞
...
---
### [wooyun-2013-037058] 联想积分商城逻辑锉刀导致支付漏洞
**厂商**: 联想 | **年份**: 2013 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2014-073305] 深信服远程任意文件中写入或清空漏洞
**厂商**: 深信服 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 深信服SangforServiceClient.exe进程存在漏洞，通过远程发送数据包，可以向指定机器的任意文件中写入文本文件或清空大于0x800任意文件
...
---
### [wooyun-2016-0172514] 融360某站点Mysql注射漏洞
...
---
### [wooyun-2012-010524] 圆通速递错误的验证码实现逻辑
**厂商**: 圆通速递官网 | **年份**: 2012 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0150288] 广东省气象局邮件系统设计逻辑缺陷获取敏感信息
**厂商**: 广东省信息安全测评中心 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
**详情**: http://**.**.**.**/抓取post信息使用常见人名，密码123456，可成功跑出登陆成功用户注册过广东省科技业务管理阳光政务平台，平台可通过邮件找回密码登陆，测试没有重置密码

**POC**: http://**.**.**.**/抓取post信息使用常见人名，密码123456，可成功跑出登陆成功用户注册过广东省科技业务管理阳光政务平台，平台可通过邮件找回密码登陆，测试没有重置密码
...
**详情**: 《360免费WiFi》app Android客户端可以免费使用移动联通电信的公共wifi，虽然有使用时间限制，但是只要成功登录上CMCC，chinanet，chinaunicom等公共wifi，通过结束后台进程就可以绕过360的时长检测免费上网。
...
---
### [wooyun-2014-061383] 华为某移动代理产品有一通用数据库账号
**厂商**: 华为技术有限公司 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
**修复**: 招行c2的支付网关貌似支持post from加sign吧再说了，没必要用get啊...
---
...
**厂商**: 中核二三集团建设公司 | **年份**: 2016 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2013-036592] 360云盘多处DLL劫持（包括win目录DLL）
**厂商**: 奇虎360 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 360WangPan.exe运行时会加载四个不存在的DLL文件分别为：dwmapi.dll，360WangPanCHS.dll，360WangPanCHT.dll，360WangPanLOC.dll假如360云盘安装在C盘劫持路径为：C:\Program Files\360\360WangPan\那么360WangPanCHS.dll，360WangPanCHT.dll，360WangPanLOC.dll这三个DLL放在该目录下，可在360WangPan.exe运行时同时加载。同时360云盘运行前还会加载系统目录下不存在的DLL文件C:\windows\system32\dwmapi.dll这四个DLL可以为自制的任意恶意dll。此方法可用于劫持启动木马病毒
...
---
### [wooyun-2012-07495] 360某平台短信轰炸漏洞
...
**详情**: 漏洞页：http://**.**.**.**/login.asp先上图。如下图：这是我国农业部畜牧业司的系统可爆破：信息：可修改：为国家做贡献。给20rank吧，我不嫌少。
...
---
### [wooyun-2012-09175] 小米MIUI漏洞可能导致硬件资源消耗
**厂商**: 小米科技 | **年份**: 2012 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 事件起源于这个漏洞，原来我一直用的软件居然是纸老虎，伤心。WooYun: 华为商城安卓客户端存储跨站&强制发短信然后猜测可能是自己手机是双卡未采用标准api来发送短信，等官方发布等版本吧LBE安全大师5.1.5446更新日志更新日期: 2013年,12月 20号下载【新增】首款支持隐私空间NFC解锁的安全软件【新增】适配双卡机型：三星I959、I9502、N9002、I929、P709 华为A199、HTC802w嗯哼，新版发布了袄支持我的手机啦，再次试了下，居然还是只能提示不能拦截。
...
**厂商**: 苏州同程旅游网络科技有限公司 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0143613] 广东省某市国土资源局办公系统设计逻辑缺陷（可导致内部信息泄露）
...
**详情**: 通过hook阿里旺旺客户端（windows版）的RECV，获取到的封包的包头为00 01 00 00 00 00 00的封包，封包里面包含对方的帐号，帐号类型，而远程地址就是对方的IP地址。
...
---
### [wooyun-2014-074966] 51信用卡管家某推广活动存在漏洞可能导致免费还信用卡
**厂商**: 51信用卡管家 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 1、手机安装51信用卡管家并注册登录，添加信用卡。点击“服务”按钮，再点击“还款金”，最后点顶部广告栏进入活动，见下图1（1.jpg）2、点击“立即充值”，填写持卡人姓名、借记卡银行、卡号，身份证号、手机号等信息。漏洞测试使用了工商银行卡，余额为0。（图2）3、下一步验证身份，任意选择一张你添加的信用卡，并填写完整信用卡号，提交申请，并点击继续充值，接着提示 余额不足，但是这个时候已经充值了0.01 元 到验证的信用卡。共尝试了4次，每次都充值1分到验证的信用卡。（图3、4、5、6）
...
---
### [wooyun-2015-0124417] 我是如何绕过中国银行网银绕过账户锁定
**厂商**: 中国银行 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
**厂商**: 北京陌陌科技有限公司 | **年份**: 2014 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0114491] 同花顺iPhone APP增值业务订购存在支付漏洞
**厂商**: 同花顺 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
**详情**: 手机APP端在订购业务时，用Burp拦截POST的数据，修改金额，然后就。。。订购成功了，晕。。。原价518元/季，现价0.01元/季，同花顺的大大们不要查我水表哈，帮忙取消这个业务就好，如果愿意赠送个一年半载就更好了，哈哈。。。O(∩_∩)O
...
**详情**: rt，vip、试题、学习卡好廉价，还有老师的题库....1）易题库域名：http://**.**.**.**/2）易题库简介：易题库是由北京易思宜学教育科技有限公司自主研发，并独创TAR在线教辅法，专注于提高教学效率、教学成果和提升学生成绩的一款革命性的智能题库产品。话说"国家资源公共教育平台"还隆重推荐呢，3）漏洞验证购买前，还不是vip，在产品里购买个vip试试，点击上图中的“开通VIP钻石版”，一直到支付页面，选择银行卡支付后再选择中国银行，这时抓到个包，修改下金额为0.01，成功跳入中国银行支付页面，后面就成功支付、买到vip了，
...
**POC**: 如需进一步信息，包括并不限于整个工程文件、更具体的系统环境、请我吃饭、讨论明清史等等，邮件发至littlebluepuppy@163.com
...
---
### [wooyun-2015-0143025] 中兴企业演练系统某功能存在逻辑缺陷可撞库爆破
...
---
### [wooyun-2015-0132528] 中信商城存在密码重置漏洞
...
**厂商**: 苏州同程旅游网络科技有限公司 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2012-014984] 联想短信验证码批量发送
**厂商**: 联想 | **年份**: 2012 | **类型**: 短信轰炸
...
**厂商**: 金山软件集团 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2014-089509] 魅族在线支付维修费用存在逻辑漏洞
**厂商**: 魅族科技 | **年份**: 2014 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2013-036787] 乐视商城支付逻辑漏洞（价格可更改）
**厂商**: 乐视网 | **年份**: 2013 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0114341] vivo手机对包名处理不当，导致无法卸载
**厂商**: vivo手机 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2015-099933] 小米手机的短信找回功能存在重放攻击风险
**厂商**: 小米科技 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2012-014777] 苏宁招行分期付款信用卡支付漏洞
**厂商**: 江苏苏宁易购电子商务有限公司 | **年份**: 2012 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0114949] 中国建设银行软键盘存在绕过风险
**厂商**: 中国建设银行 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2012-015581] 喜闻乐见的1分钱车票+顺手发现的银联和邮储银行支付漏洞
...
**详情**: 在阿里旺旺2012中，使用短信旺旺中心对短信条数进行充值，在该过程中，未使用SSL安全连接，同时存在支付漏洞。选择充值的金额后，下一步，使用FID截取数据包，GBK编码，修改POST数据中的数据，seriesPrice为支付的金额，seriesNum为充值的条数，这两个地方理论上都可以进行修改，这里以修改seriesPrice为例。
...
---
### [wooyun-2013-018597] 360桌面专业版可能导致屏锁密码一些问题
**厂商**: 奇虎360 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 总的意思来说，安装360桌面的一个屏锁功能之后，设置的访问密码就自动消除了卸载了之后 工作正常测试：小米手机。
...
**厂商**: 苏州同程旅游网络科技有限公司 | **年份**: 2015 | **类型**: 账户体系控制不严
...
---
### [wooyun-2015-093988] 顺丰优选支付逻辑漏洞
**厂商**: 顺丰优选 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
**详情**: 下载顺丰优选app应用，随便购买两件商品，商品为数量一正一负提交订单。提交订单时可能会报错。将两个参数的正负数调换可成功绕过。
...
**详情**: 阿里旺旺最新版（版本8.00.40C），官方下载安装版本AliIM2014_taobao(8.00.40C).exe，windows xp sp4环境，阿里旺旺的主程序AliIM.exe在运行时会加载两个不存在的dll文件（该文件没有随阿里旺旺一起安装），导致恶意攻击者可以伪造该DLL文件用于劫持，严重时可危害用户的安全，如帐号窃取、网络钓鱼等。两个DLL文件的对应路径如下：C:\Program Files\AliWangWang\8.00.40C\modules\1685\WWPictureUpload.dllC:\Program Files\AliWangWang\8.00.40C\wwsdkcom.dll
...
---
### [wooyun-2015-0114435] 360安全浏览器设计不当可导致用户保存密码被窃取
**厂商**: 奇虎360 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 搜狗浏览器的主程序在运行中浏览某些特定网页(如搜狐视频)后关闭页面时会加载一个不带路径的aticfx32.dll文件（该文件没有随搜狗浏览器一起安装），导致恶意攻击者可以伪造该DLL文件用于劫持，严重时可危害浏览器用户的安全，如帐号窃取、网络钓鱼等。windows环境其他没测试过咯搜狗浏览器（版本5.2.5.16447）不存在dll：aticfx32.dll运行程序：SogouExplorer.exe目录：F:\搜狐浏览器\SogouExplorer
...
---
### [wooyun-2015-0154369] 我想买个手表之乐视商城支付漏洞
**厂商**: 乐视网 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
**厂商**: 金山软件集团 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2013-024117] 凤凰网支付漏洞可无限获得凤凰币
**厂商**: 凤凰网 | **年份**: 2013 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2014-062558] 魅族flyme验证码绕过可暴力破解，查看短消息、通讯录，远程拍照、发消息
**厂商**: 魅族科技 | **年份**: 2014 | **类型**: 账户体系控制不严
...
**详情**: 魅族在自己的服务器上保存很多用户资料：通讯录、短信、位置信息等。但他们对用户的数据保护却做得很差。下午一位朋友的手机被他人锁定了，怀疑是账号被盗。简单测试，发现flyme的服务竟然可以猜解用户名，无限暴力破解。验证码直接绕过。。。用双拼音做用户名，123456、654321、111111这几个密码，可以破解出来不少账号。而登录flyme之后，竟然可以直接查看用户的通讯录、短信记录。甚至远程拍照、定位、发送消息、播放声音等。我和我的小伙伴都惊呆了。。
...
---
### [wooyun-2015-0136129] TCL内部邮箱密码泄露及某处逻辑支付漏洞(影响公司财产安全)
**厂商**: TCL官方网上商城 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0122148] 向上360某功能逻辑缺陷可以任意重置用户密码
...
---
### [wooyun-2014-084180] 优酷网某后台验证码绕过(破解管理员两枚)
...
---
### [wooyun-2015-0129987] 中兴某业务网站支付逻辑漏洞导致任意低金额充值高额度
**厂商**: 中兴通讯股份有限公司 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2013-042313] 飞马网逻辑缺陷可刷学分
...
---
### [wooyun-2014-086215] 交通银行业务流程审核逻辑不严导致可枚举任意信用卡cvv码
**厂商**: 交通银行 | **年份**: 2014 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2013-036825] 工商银行网银助手多处DLL劫持
**厂商**: 中国工商银行 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 首先进入易拍网，网址：www.yeepai.com登陆账号点充值点击充值￥   1000   =    100000  拍点 + [ 赠送 20000 返点 + 再送 60元代金券 ] 的付款银行点击工商银行，然后修改！将1000修改为1,，下面提示就成了充值1元，1百拍点了，但是充值完成之后的20000返点依然会默认为充值1000块钱的额度赠送，也就是我们充值1元钱就可以享受充值1000的待遇了！生成订单看看！赠送返点： 	20000点  果然是20000点！
...
**POC**: 其实就是利用了NTFS的硬链接特性，利用同一个磁盘文件的不同文件名和路径来规避常规的安全软件检测。fsutil hardlink create c:\a.txt C:WINDOWS\system32\drivers\etc\hosts执行此命令后c:\a.txt与C:WINDOWS\system32\drivers\etc\hosts两个文件实际上是等价的，在硬盘上是同一个文件，只是拥有两个文件名而已，利用这一点可以绕过一些常规安全软件的防护。有了这个特性，我们就可以任意读写大多数文件了。下面是对360的吐槽图：提交至360SRC时的证明截图：而昨晚的测试中，最新版360安全卫士变成了直接拦截
...
---
### [wooyun-2015-092578] TCL商城一处支付漏洞
**厂商**: TCL官方网上商城 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
**详情**: 被封停的旺旺账号在阿里旺旺上面无法登录但是可以再千牛上面登录旺旺并且拥有和旺旺一样的功能可以正常进行聊天和其他动作
...
---
### [wooyun-2015-0112962] 招商银行网上银行多个客户端在低版本Windows下存在DLL劫持漏洞（个人银行，电子商务）
**厂商**: 招商银行 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2015-0161233] 海尔某社区逻辑漏洞泄露10多万账户信息
**厂商**: 海尔集团 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2011-02272] 顺丰宝业务逻辑漏洞
**厂商**: 顺丰宝 | **年份**: 2011 | **类型**: 设计缺陷/逻辑错误
...
**详情**: 顺丰宝存在支付逻辑漏洞，可以允许用户1元变1亿元。这个漏洞在其他网站很难存在，原因是页面交互都使用了对字段做签名。但是顺丰宝没做签名，导致支付金额可以被修改为任意数值。猜测成因是开发人员为了快速实现功能，而忽略了其中数据签名的步骤。可以想象，如果我充值1个亿，然后再使用取款功能，会产生神马效果。

**POC**: 1、登陆就不说了，来到充值页面。我的余额为1元。2、充值，选择招商银行。提交看到一个页面。3、确认这个页面，同时开启修改数据包。修改数据包中的金额，为1分。4、提交。招行付账。5、成功！招行扣了1分钱。6、查询顺丰宝，充值了1元。
...
---
### [wooyun-2013-039716] 小米科技某合作游戏BUG导致可秒杀高等级玩家
**厂商**: 小米科技 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 小米科技公司与莱茵天下（北京）有限公司合作的游戏小米部落存在设计错误。漏洞1.日常任务可以通过修改本地内存的数据，以达到不用作任务，就可以领取日常任务中的奖历。任务未完成前，不可以领取奖历，通过内存数据修改，将要达到的值如100，120，...200，这些数据改成0，或比自己小的数据，然后就可以直接领取了。领取后如图===================================漏洞2.秒对手。同样的道理，可以通过修改本地内存的方式，使游戏本身人员的攻击，血里增加，这里改成999999，比对方高好几十倍，以达到秒杀比自己高几十级的玩家，并可以轻松的解救对方的奴隶。
...
---
### [wooyun-2015-094635] 优酷Android 4.5客户端升级漏洞
**厂商**: 优酷 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2015-0160699] 格力某网站设计逻辑缺陷导致信息泄露
**厂商**: 格力 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0135164] 苏宁易购主站逻辑缺陷不花钱买买买
**厂商**: 江苏苏宁易购电子商务有限公司 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0130345] 华为莫塞尔商城之绕过短信验证码并任意重置密码
...
---
### [wooyun-2012-04930] 建设银行android客户端设计逻辑缺陷导致用户被钓鱼
**厂商**: 建设银行 | **年份**: 2012 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2015-0144604] 同花顺爱基金客户端之本地用户信息明文存储
**厂商**: 同花顺 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2013-036531] 金山猎豹安全浏览器DLL劫持漏洞
**厂商**: 金山网络 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 金山猎豹安全浏览器最新版（版本4.2.25.5405），官方下载安装版本KSbrowser_4.2.25.5405.exe，windows xp sp3环境，金山猎豹安全浏览器的主程序在运行中浏览某些特定网页时会加载一个位于用户数据目录下的DLL文件(%USERPROFILE%\Local Settings\Application Data\liebao\User Data\common\kcontent_factory.dll，其中common目录安装完猎豹默认没有创建，可手工创建)，用户数据目录不被猎豹的安全防御系统所保护，导致可能被恶意攻击者利用进入其进程空间实现提权及其它恶意行为。测试发现对以前的版本同样有效。
...
---
### [wooyun-2015-0144617] 同花顺手机客户端账号登录密码MD5值SDcard目录下明文存储
**厂商**: 同花顺 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 同花顺 Android客户端 账户信息明文存储在SDcard目录文件中，任何应用只要有读写sdcard权限，都可读取同花顺客户端存储的账户信息。

**POC**: 被测Android客户端下载地址：http://**.**.**.**/apk  : com.hexin.plat.android, 4205, V9.10.02, 同花顺apkmd5: 6a2abaeb771557de489681187b1ffb8d证书 :certmd5: 110582f43caed505854599c7ada28e77issuer: C=CN, ST=ZheJiang, L=Hangzhou, O=ShouJiKaiFaZu, OU=TongHuaShun, CN=hexinsubject: C=CN, ST=ZheJiang, L=Hangzhou, O=ShouJiK
...
---
### [wooyun-2015-096706] 优酷客户端存在可信任路径代码执行漏洞
**厂商**: 优酷 | **年份**: 2015 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 每一次打开优酷客户端时，都会打开一个来自exe文件所在盘的根目录下的program.exe。测试版本：打开客户端时：
...
---
### [wooyun-2015-0148542] 小米某智能家电存在漏洞可被渗透导致整个智能家居网络沦陷（无线局域网测试案例）
**厂商**: 小米科技 | **年份**: 2015 | **类型**: 设计不当
...
**修复**: 1、小米投屏神器仅能在登录状态下访问本帐号绑定的小米盒子。2、小米盒子增加认证机制。3、小米盒子可设置连接设备黑白名单。
---
...
---
### [wooyun-2014-058717] 中国农业银行网银体验机绕过
**厂商**: 中国农业银行 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
---
### [wooyun-2012-016193] 交通银行免费买车漏洞
**厂商**: 交通银行 | **年份**: 2012 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0151344] 某大学网银收费系统设计逻辑缺陷可导致学生信息泄漏
**厂商**: CCERT教育网应急响应组 | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2015-0108637] 天天果园某接口撞库泄露用户登录凭据(验证码绕过有账号证明)
**厂商**: fruitday.com | **年份**: 2015 | **类型**: 设计缺陷/逻辑错误
...
---
### [wooyun-2013-037783] 联想网络控制工具(被控端)暴露硬盘保护卡密码
**厂商**: 联想 | **年份**: 2013 | **类型**: 设计错误/逻辑缺陷
...
**详情**: 上面的界面就是联想网络控制工具(被控端)退出时的界面，需要输入密码。你随意输入一个错误的密码，通过OllyDBG使用字符串类的函数如lstrlenA下断点，可以跟踪出正确的明文密码出来。我实际测试的是EDU 7.6版本。
...
---
### [wooyun-2014-050767] 小米手机加密文件管理器可通过第三方软件绕过验证
**厂商**: 小米科技 | **年份**: 2014 | **类型**: 设计错误/逻辑缺陷
...
**修复**: 再升级新版本的时候修复一下吧，我也挺支持小米的，可是我发现那么多人提交了小米科技实在的漏洞，全被你们忽略了，小米的手机确实挺强，但研发高科技的同时也不要忽视了安全，也就相当于忽视了用户的安全，感觉失望，在这样的话哪个好心的白帽子还会重视小米科技的安全检测？看在我一下子买了两台小米2S的份上不要在忽略
---
